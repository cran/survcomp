`logpl` <-
function(surv.time, surv.event, beta, x, strata.cox=NULL, na.rm=FALSE, verbose=FALSE) {

	##############
	#internal function
	##############
	
	logpl1 <- function(surv.time, surv.event, beta, x, verbose=FALSE) {	
		
		x <- as.matrix(x);
		if (length(surv.time) == 1 && length(surv.event) == 1) { x <- t(x); }
	...
	x <- as.matrix(x)
	beta <- as.matrix(beta)
	cc.ix <- complete.cases(surv.time, surv.event, x, strata.cox);
